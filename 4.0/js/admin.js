...
function extractPreferredCut(notes, booking = null) {
  ...
  if (booking && booking.referenceCut) {
    ...
    // Check if it's a cut ID (starts with 'cut-')
...
async function initAdminDashboard() {
  ...
  const loaderId = showDashboardLoader('admin');
  ...
  try {
    ...
    // Load overview with comprehensive error handling
    try {
      await loadOverview();
    } catch (overviewError) {
      ...
      // Don't fail the entire dashboard if overview fails
      // Show empty state instead
      updateStatsCards(0, 0, 0, 0, 0, 0);
      renderRecentBookings([]);
    }
    
    // Update notification badge (non-critical)
    try {
      updateNotificationBadge();
      // Refresh notifications less frequently if there are permission issues
      const refreshInterval = hasPermissionIssues ? 120000 : 30000; // 2 minutes vs 30 seconds
    ...
    loadingManager.showError(errorMessage + ' You can try refreshing the page.');
  } finally {
...
async function loadPendingBookings() {
  ...
  // 🔧 DEBUG: Log all bookings to identify duplicates or missing bookings
  console.log('[loadPendingBookings] Total bookings in system:', bookings.length);
  console.log('[loadPendingBookings] All booking IDs:', bookings.map(b => b.id));
  ...
  console.log('[loadPendingBookings] Pending bookings count:', pendingBookings.length);
  console.log('[loadPendingBookings] Pending booking details:', pendingBookings.map(b => ({
    id: b.id,
    customerName: b.customerName,
    date: b.date,
    time: b.time,
    hasPaymentProof: !!b.proofOfPaymentImage
  })));
  ...
  showModal(`
    ...
    ${(() => {
      ...
      if (booking.referenceCut) {
        ...
        preferredCutHtml = `<p><strong>Preferred Cut:</strong> <span style="background: #e8f5e9; padding: 0.25rem 0.5rem; border-radius: 0.25rem; color: #2e7d32; font-weight: 600;">✂️ ${escapeHtml(cutDisplayName)}</span></p>`;
      }
      ...
      if (booking.bookingNotes && booking.bookingNotes.trim()) {
        ...
        if (notesToDisplay) {
          notesHtml = `<p><strong>Special Notes:</strong> <span style="background: #fff9e6; padding: 0.25rem 0.5rem; border-radius: 0.25rem; color: #f57c00; font-weight: 500;">📝 ${escapeHtml(notesToDisplay)}</span></p>`;
        }
      }
