...
async function submitBooking(event) {
  ...
  // Prevent duplicate submissions with cooldown
  const now = Date.now();
  if (isSubmittingBooking || (now - lastSubmitTime) < SUBMIT_COOLDOWN_MS) {
    const timeLeft = Math.ceil((SUBMIT_COOLDOWN_MS - (now - lastSubmitTime)) / 1000);
    console.log('[submitBooking] Cooldown active, ignoring request. Time left:', timeLeft, 'seconds');
...
async function assignFairGroomer(date, time) {
  try {
    ...
    const groomerStats = groomers.map(g => {
      // Total bookings for the day (exclude cancelled)
      ...
      // Total CONFIRMED bookings (all time) - only count confirmed, not pending
    ...
    // Sort by: 
    // 1. Fewest picks on DAY BEFORE BOOKING (primary - for fair rotation)
    // 2. Fewest bookings for this time slot (secondary)
    // 3. Fewest bookings for the day (tertiary)  
    // 4. Fewest total CONFIRMED bookings (quaternary - for ultimate fairness)
    // 5. Random selection if still tied (quinary)
    candidates.sort((a, b) => {
      // Primary: Day before booking's picks (least picked = 1st priority)
