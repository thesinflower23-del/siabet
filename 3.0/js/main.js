...
const DEFAULT_REFERENCE_CUTS = [
  ...
  { 
    ...
    image: 'assets/cuts/teddy-bear-cut.jpg',
    petTypes: ['dog'],
    isActive: true,
    sortOrder: 2
  },
  { 
    ...
    image: 'assets/cuts/lion-cut.jpg',
    petTypes: ['dog', 'cat'],
    isActive: true,
    sortOrder: 3
  },
  { 
    ...
    image: 'assets/cuts/summer-cut.jpg',
    petTypes: ['dog', 'cat'],
    isActive: true,
    sortOrder: 4
  },
  { 
    ...
    image: 'assets/cuts/kennel-cut.jpg',
    petTypes: ['dog'],
    isActive: true,
    sortOrder: 5
  },
  { 
    ...
    image: 'assets/cuts/show-cut.jpg',
    petTypes: ['dog', 'cat'],
    isActive: true,
    sortOrder: 6
  }
...
function initializeReferenceCuts() {
  ...
  if (!stored) {
    localStorage.setItem('referenceCuts', JSON.stringify(DEFAULT_REFERENCE_CUTS));
    console.log('[ReferenceCuts] Initialized with defaults');
  }
}
...
function saveSubmissionStatus(status) {
  ...
  console.log('[SubmissionStatus] Status saved:', status.status, status.token);
}
...
function startSubmission(userId) {
  ...
  console.log('[SubmissionStatus] Submission started:', token);
  return token;
}
  ...
  try {
    ...
    bookings.forEach(booking => {
      ...
      if (bookingDate === today) {
        ...
        if (bookingDateObj <= now) {
          ...
          if (now.getHours() > cutoffHour || 
              (now.getHours() === cutoffHour && now.getMinutes() >= cutoffMinute)) {
            shouldCancel = true;
            // Future booking should be cancelled (reduced logging)
          }
        }
      }
      ...
      if (shouldCancel) {
        ...
        // Booking cancelled (reduced logging)
      }
    });
    ...
    if (hasChanges) {
      await saveBookings(bookings);
      // Bookings updated (reduced logging)
      ...
      if (cancelledBookings.length > 0) {
        console.log(`[AutoCancel] Processed ${cancelledBookings.length} expired booking(s)`);
      }
    }
  } catch (error) {
    ...
    <article class="review-card">
      ...
      if (entry.referenceCut) {
        ...
        if (cutId.startsWith('cut-')) {
          // Use getReferenceCutName to get display name from ID
